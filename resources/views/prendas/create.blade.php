...
@section('content')
        ...
        <form action="{{ route('prendas.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-[#1b1d1a] p-5 sm:p-8 rounded-lg border border-[#f3f2ec]/10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-[#f3f2ec]/70 mb-2" style="font-family: 'Space Mono', monospace;">
                        Talla <span class="text-[#c8ff00]">*</span>
                    </label>
                    <x-size-selector :tallas="$tallas" :selected="old('talla')" name="talla" />
                    @error('talla')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
